...
def main():
    ...
    # 2. Load SD-LoRA (Optional)
    ai_pipe = None
    if args.lora_path:
        print("⏳ Loading Stable Diffusion LoRA...")
        model_id = "runwayml/stable-diffusion-v1-5"
        ai_pipe = StableDiffusionPipeline.from_pretrained(model_id, torch_dtype=torch.float16).to(device)
        ai_pipe.load_lora_weights(args.lora_path)
        # ai_pipe.enable_xformers_memory_efficient_attention()
    ...
    for i in tqdm(range(min(args.num_test, len(clean_files)))):
        ...
        if ai_pipe:
            # Note: In a full test, we'd use the damaged image as a guide/prompt
            # For now, we generate based on the restoration prompt
            prompt = "a perfectly restored antique photo, high detail, sharp"
            ai_gen = ai_pipe(prompt, num_inference_steps=20).images[0]
        ...
        cv2.putText(grid, f"PSNR: {psnr:.1f}dB", (w * 2 + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
